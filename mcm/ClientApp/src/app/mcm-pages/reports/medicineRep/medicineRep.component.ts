...
@Component({
  selector: 'reports-medicine',
  styleUrls: ['../reports.component.css'],
  templateUrl: './medicineRep.component.html'
})

export class MedicineRepComponent {
  public state: State = {
    filter: {
      logic: 'and',
      filters: []
  ...
  FilterByStatus(data, _status) {
    var newStatus = parseInt(_status) === 1 ? true : false;
    if (parseInt(_status) !== 2) {
  ...
  GetMedicineReports() {
    ...
    var sub = this.service.GetMedicineReports().subscribe(data => {
      var service = this.util;
      _.forEach(data, function (d) {
        d.med_received = service.FormatNumber(d.med_received);
        d.med_issued = service.FormatNumber(d.med_issued);
        d.med_onhand = service.FormatNumber(d.med_onhand);
      })
